...
| Oracle | Two independent sources per branch, both fixed: an external ETH/USD feed, and the liquidity-weighted median of the time-weighted prices of fixed third-party WETH/stablecoin pools. Either alone keeps the branch live; when both answer they must agree; a shutdown needs both to fail or a lasting disagreement. Sequencer guard; temporary states before a permanent failure; fixed gas stipends. The protocol's own pool is never a price source. | decided |
...
| Gas deposit per Trove | 0.001 ETH | §9.3 |
| Oracle, primary: staleness threshold / failure timeout / grace / feed stipend / sequencer-guard stipend | 1 h (3 × the 20-minute heartbeat) / 24 h / 1 h / 100 000 gas / 100 000 gas | §7 |
| Oracle, pool source (O7): pools | Uniswap v3 WETH/USDC 0.05 % and 0.3 %, Aerodrome Slipstream WETH/USDC and WETH/USDT (tick spacing 100), on Base | §7 |
| Oracle, pool source: window `W` / `POOL_STALENESS` / `MIN_DEPTH` / `MAX_DEVIATION` / source stipend / per-call stipend | 10 min / 1 h / 5 000 000 (weight, in dollars) / 5 % / 600 000 gas / 80 000 gas | §7 |
...
- **O1** One `IPriceFeed` per branch, fixed at creation; the protocol cannot replace it. A feed has a primary source (an external feed) and optionally a pool source (O7), both fixed at the feed's construction. The protocol's own pool, and any pool the protocol holds liquidity in, is never read. (deployment, S-26)
- **O2** Status order: `NetworkUnstable` (sequencer down or within the grace period) → `PriceInvalid` (stale, non-positive, future-dated, unreadable, composite parts too far apart, or the two sources disagreeing, O8) → `Failed` → `Valid`. (S-20 a–i, fuzz_oracle F1–F9, M-12, M-13, M-14)
- **O3** `Failed` only with the sequencer continuously up for the whole timeout, and either (a) both sources dead: the primary with a readable answer older than the timeout or with malformed answers observed a timeout apart and no healthy observation between, and the pool source absent or unavailable at observations a timeout apart with no available observation between; or (b) the two sources observed disagreeing a timeout apart with no `Valid` observation between. A flapping sequencer postpones `Failed`. (S-20, S-36, M-11, M-15, M-52, M-54, M-59)
...
- **O5** Gas stipend: each feed has an immutable `FEED_GAS_LIMIT`; before the read the transaction proves `gasleft ≥ (stipend + overhead) × 64/63 + buffer`; the read is a low-level `staticcall` copying exactly 64 bytes. Assumptions: stipend above the feed's true cost (the stipends are §2 constants; a cold read of the live Base feed, of its sequencer guard and of the four-pool source is measured on a fork against them, Foundry `test_fork_theLiveFeedIsValidAndOneReadFitsTheStipend`, `test_fork_thePoolSourceAgreesWithTheFeedAndThinPoolsWeighNothing`). Every transaction that reads the price needs a gas limit above the stipends, whatever it uses, adapter returns the 64-byte shape. A provider swapping in a costlier aggregator can turn the stipend into a detected failure of the primary, which the pool source then covers (O8). The pool source is read with its own stipend, and inside it each pool with a fixed stipend of its own. (S-20 e/f, Foundry `test_e_stipendGuardCannotBeGriefed`, `test_f_gasBurningSource`)
- **O6** After a `Failed` shutdown all later prices are `lastGoodPrice`. (S-15a)
- **O7** The pool source reads a fixed set of at most four third-party WETH/stablecoin concentrated-liquidity pools through their `observe` oracle, over a fixed window `W`. Per pool: mean tick `t = floor(Δtick / W)` (toward −∞), with WETH as the base (the tick negated when WETH is token1); harmonic mean liquidity `L = W·(2¹⁶⁰ − 1) / (ΔsecondsPerLiquidityX128 · 2³²)`; `s = sqrtPriceAtTick(t)`; price `p = ((s·s / 2⁶⁴) · 10^(36−d)) / 2¹²⁸` and weight `w = (L·s / 2⁹⁶) · 10^(18−d)`, the pool's virtual stablecoin reserve at that price, with `d` the stablecoin's decimals (every division rounds down). A pool is left out when a call to it fails within its fixed gas stipend or its last observation (a pool records one only when its tick or its liquidity changes) is older than `POOL_STALENESS`. The source's price is the weighted median: with the pools sorted by price (ties in construction order), the first price at which the cumulative weight reaches half of `Σ w`; the source is unavailable when `Σ w < MIN_DEPTH`. A mean of prices is not used because a price is unbounded: a pool drained to an extreme tick keeps a tiny weight and would still move a mean arbitrarily far, while pools holding less than half the weight cannot move the median at all, and a pool left without liquidity even for seconds of the window loses almost all its time-weighted liquidity. Each stablecoin is taken at one dollar. (S-36, M-55, M-56, M-61, M-62, M-63, Foundry `test_diff_poolTwapMathMatchesModel`, `test_aPoolIsLeftOutWhenStaleOrFailingAndTheDepthFloorHolds`, `test_fork_thePoolSourceAgreesWithTheFeedAndThinPoolsWeighNothing`)
- **O8** After the network check of O2: a healthy primary (well-formed, not stale) gives `Valid` at the primary's price, unless the pool source is available and `max(pA, pB) > min(pA, pB) · (1 + MAX_DEVIATION)`, which is `PriceInvalid`. A dead primary (O3's conditions for the primary, with the sequencer up for the timeout) gives `Valid` at the pool source's price when it is available, `Failed` when it is dead too, `PriceInvalid` otherwise. A primary that is only temporarily bad gives `PriceInvalid` whatever the pool source says. Markers: `invalidSince` is cleared only by a healthy primary, `poolInvalidSince` only by an available pool source, `disagreeSince` by any `Valid`; each is set by the first observation of its condition and survives only in non-reverting transactions (O4). Without a pool source the procedure is exactly the single-source one. The redemption price is the price. (S-36, M-50, M-51, M-53, M-57, M-58, fuzz_oracle F10–F13, Foundry `test_diff_oracleMatchesModel`, `test_fork_aDeadFeedFallsBackToThePoolsInsteadOfShuttingDown`)
...
- **V6** The liquidity vault (peripheral, outside the core) accounts swap fees per token and any reward tokens paid into it with the same fixed-epoch stream; a just-in-time entrant earns only its seconds; time with no shares rolls forward. The fees a change of shares collects from the pool belong to the holders before the change; principal is never mixed into the fee books. (S-19, Foundry `test_diff_lpAccountingMatchesModel`, `test_fork_theVaultEarnsSwapFeesAndANewcomerGetsNoneOfTheOld`)
- **V7** The vault pays each token on its own, to a recipient the holder names. A withdrawal credits its principal and pays nothing; `claim(k, to)` pays the holder's principal and earnings in token `k` alone. What the vault owes in the pool's two tokens it holds as claims inside the PoolManager, never as token balances, and a payout moves the token from the PoolManager straight to the recipient. So a token that refuses a transfer to or from an address (a blacklist) blocks only payouts of that token to that recipient: never a withdrawal of shares, never another token, never another holder, and not even when the address refused is the vault's own. (S-19, M-49 one claim empties every token's books, Foundry `test_diff_lpAccountingMatchesModel`, `test_fork_aFrozenAddressBlocksOnlyItsOwnPayoutOfThatToken`)
...
- The cold gas of X2, X3, X5 and of a liquidation is bounded in Foundry `test_settlingAndWritingOffATroveCostWhatTheDepositIsSizedFor`, `test_aWriteOffCostsWhatHalfTheDepositIsSizedFor`, `test_aLiquidationCostsWhatTheDepositAndTheBonusAreSizedFor`; the deposit (§2) is sized against those bounds and a year of Base gas prices (RESULTS).
- **[open]** Behaviour when `_touch` reverts persistently; a written-off Trove that can never be settled keeps its own collateral stuck.
...
- **D2** The pool race: the token's address is predictable and v4 lets anyone initialise any key, so deployment must not depend on winning. If `initialize` succeeds, the pool must hold exactly the target price, or the deployment reverts: every uncontested deployment checks the storage layout on the real PoolManager. If `initialize` fails, the pool must demonstrably exist and hold a price v4 can hold; otherwise the deployment reverts. The layout matches v4-core v4.0.0 and the PoolManager deployed on Base (fork at block 51,380,224): an uncontested deployment reads back its own price, a raced one the attacker's. Target and observed prices are recorded separately. (S-26, Foundry `test_fork_anUncontestedDeploymentReadsBackItsOwnPrice`, `test_fork_aRacedPoolIsToleratedItsPriceCorrectedForFreeAndTheVaultGuardHolds`, Foundry `test_poolRace_realPredictedAddress`, `test_unrelatedInitialiseFailure_revertsInsteadOfFalseSuccess`, `test_uncontestedDeployment_readsBackItsOwnPrice_orRefuses`, `test_racedPool_priceNoPoolCanHold_isRefused`)
- **D3** The core never holds a reference to Uniswap, the pool or the vault. (S-26)
- **D4** The vault refuses deposits while the pool price is outside its fixed range, and each depositor states his own accepted price bounds and maximum amounts. This is not protection against manipulation inside the band. The price of a pool without liquidity can be moved by anyone at no cost, so a raced pool is corrected by the first depositor in the same transaction. The position maths are Uniswap v4's own, on the real PoolManager; the model covers the vault's accounting (V6). (S-26, Foundry `test_fork_anUncontestedDeploymentReadsBackItsOwnPrice`, `test_fork_aRacedPoolIsToleratedItsPriceCorrectedForFreeAndTheVaultGuardHolds`, `test_fork_depositsAreRefusedOutsideTheRangeAndOutsideTheDepositorsBounds`, `test_theRangeBracketsParForEitherTokenOrderAndAnySpacing`)
...
independent computation (S-30); the oracle properties F1–F13 (`fuzz_oracle.py`).
...
| Oracle | 20, 36 | M11–M15, M50–M59, M61–M63 | fuzz_oracle F1–F13 |
...
1. DARLI supply and distribution. 2. Legal review before any deployment.
