...
- **O5** Gas stipend: each feed has an immutable `FEED_GAS_LIMIT`; before the read the transaction proves `gasleft ≥ (stipend + overhead) × 64/63 + buffer`; the read is a low-level `staticcall` copying exactly 64 bytes. Assumptions: stipend above the feed's true cost (one cold read of the live Base feed and of its sequencer guard is measured on a fork, Foundry `test_fork_theLiveFeedIsValidAndOneReadFitsTheStipend`; the stipend's value is **[open]**), adapter returns the 64-byte shape. A provider swapping in a costlier aggregator can turn the stipend into a detected failure of the primary, which the pool source then covers (O8). The pool source is read with its own stipend, and inside it each pool with a fixed stipend of its own. (S-20 e/f, Foundry `test_e_stipendGuardCannotBeGriefed`, `test_f_gasBurningSource`)
