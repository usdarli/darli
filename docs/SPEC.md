...
- **R4** Two prices: `price` decides redeemability (ICR ≥ 100 %; a Trove below is skipped and counts as an iteration); `redemptionPrice` (conservative) converts debt to collateral. With the two prices equal, as for plain collateral, an ordinary redemption never lowers the ICR of a Trove at or above 100 %. With `redemptionPrice` below `price` that holds only for a Trove at or above 100 % at `redemptionPrice` **[open, §13]**. (I-8, Foundry `test_aTroveUnder100PercentIsSkippedButCountsAsAnIteration`, `test_debtIsConvertedAtTheRedemptionPriceAndTheFeeStaysInTheTrove`, `testFuzz_aRedemptionNeverLowersTheIcrOfATroveAt100PercentOrMore`)
...
1. β. 2. Gas deposit amount. 3. Fixed values of `FEED_GAS_LIMIT` and oracle thresholds from a fork test. 4. PoolManager storage layout for the post-initialise check: matches v4-core source; to be confirmed against the deployed bytecode on a fork. The uncontested path already checks itself (D2). 5. Vault quote asset, range and position maths. 6. DARLI supply and distribution. 7. Persistent failure in shared settlement parts. 8. Legal review before any deployment. 9. A branch whose `redemptionPrice` is below its `price` (a composite feed; not the WETH branch of version 1): redeemability is decided at `price` (R4), so a Trove at or above 100 % there but below 100 % at `redemptionPrice` could have its ICR lowered by a redemption (the fee offsets part of it), or, redeemed in full, owe more collateral than it holds, which reverts the whole redemption. Deciding redeemability at `redemptionPrice` as well is the proposed remedy; it needs a rule, a scenario and a mutant first.
