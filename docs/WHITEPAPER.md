...
**What staged settlement costs.** Until phase 1 ends, which takes one transaction per Trove and, for Troves nobody processes, at least thirty days, no holder can turn USDarli into collateral. A borrower can no longer escape a stale reference price by repaying: after an oracle-failure shutdown the last good price binds borrowers as well as holders. And holders carry the market risk of the collateral from the shutdown until they claim: in the simulation the worst recovery in the 40% cases was <!-- fig:studies.pilot_worst_recovery_40pct_cases -->0.96<!-- /fig --> for that reason alone, with no shortfall at all.

Valued at a nominal one dollar per USDarli, the liquidity provider in the worst run shows a result of <!-- fig:studies.pilot_lp_worst_run_nominal_pct -->+0.8<!-- /fig -->%; at settlement value the same position lost <!-- fig:studies.pilot_lp_worst_settlement_loss_pct -->23.2<!-- /fig -->%. The upper-side keeper treasury of <!-- fig:studies.keeper_budget_usd -->40,000<!-- /fig --> dollars, modelled as a real ledger of ETH and cash, was short for <!-- fig:studies.keeper_blocked_hours_calm_mean -->19<!-- /fig --> hours per run on average in calm markets and for <!-- fig:studies.keeper_blocked_hours_crash_mean_min -->25<!-- /fig --> to <!-- fig:studies.keeper_blocked_hours_crash_mean_max -->68<!-- /fig --> hours in the crash cases. Its result averaged between <!-- fig:studies.keeper_pnl_case_mean_min_usd -->+85<!-- /fig --> and <!-- fig:studies.keeper_pnl_case_mean_max_usd -->+1,082<!-- /fig --> dollars per case, and ranged from <!-- fig:studies.keeper_pnl_single_run_min_usd -->-60<!-- /fig --> to <!-- fig:studies.keeper_pnl_single_run_max_usd -->+7,841<!-- /fig --> dollars in single runs. All figures in this table are under staged settlement with healthy borrowers absorbing first; in the worst case every holder shared the loss at the same rate. These are outcomes of an uncalibrated simulation of one pool and one collateral. They show where losses land and roughly how large they are, not what will happen.
