...
1. `python3 model/test_scenarios.py`, `test_econ_sim.py`, `fuzz.py 30 300`, `fuzz_oracle.py 200 150`, `mutants.py`, `spec_check.py`, `check_manifest.py`, `check_figures.py`: all green on the release commit, and `git diff --exit-code -- model/results/figures.json docs/RESULTS.md` clean afterwards (the record was regenerated, not edited).
