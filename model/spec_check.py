...
every Foundry test SPEC.md cites exists as a function in contracts/test or contracts/fork (it cited `test_e` and `test_f` for years; neither existed).
...
import pathlib
foundry_src = "".join(f.read_text(encoding="utf-8") for d in ("../contracts/test", "../contracts/fork")
                      for f in sorted(pathlib.Path(d).rglob("*.sol")))
...
bad += [f"Foundry `{t}` cited but no such test in contracts/test or contracts/fork" for t in sorted(cited_f) if t not in foundry_fns]
